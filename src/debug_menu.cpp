...
constexpr int k_y_step = GEX_POS( 15 ); // vertical step between lines
...
{
    current_menu->seek_endmenu( )->data1 = selected_index;
    menu_stack.push( current_menu );
    current_menu = a_menu;
    selected_index = current_menu->seek_endmenu( )->data1;
}
...
void dm_line::draw( int a_y, bool a_selected )
{
    ...
    case line_type::integer:
    {
        ...
        {
            if( *p )
                TXT_DrawPrintF( GEX_POS( 200 ), a_y, "%d", **p );
            else
    ...
    case line_type::boolean:
    {
        ...
        if( boolopt.has_value( ) )
            TXT_DrawPrintF( GEX_POS( 200 ), a_y, "%s", *boolopt ? "true" : "false" );
        else
            TXT_DrawPrintP( GEX_POS( 200 ), a_y, "NULL" );
    ...
    case line_type::flag:
    {
        ...
        {
            if( *p )
                TXT_DrawPrintF( GEX_POS( 200 ), a_y, "%s", ( ( **p & data1 ) != 0 ) ? "true" : "false" );
            else
                TXT_DrawPrintP( GEX_POS( 200 ), a_y, "NULL" );
        }
        break;
...
menu_builder& menu_builder::add_menu( std::string_view menu_name, dm_line* menu )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::menu, std::string( menu_name ), menu };
    return *this;
}

menu_builder& menu_builder::add_action( std::string_view text, dm_action_fun func, int arg )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::action, std::string( text ), func, arg };
    return *this;
}

menu_builder& menu_builder::add_int_ref( std::string_view text, int* val )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::integer, std::string( text ), val };
    return *this;
}

menu_builder& menu_builder::add_bool_ref( std::string_view text, int* val )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::boolean, std::string( text ), val };
    return *this;
}

menu_builder& menu_builder::add_bool_ref( std::string_view text, bool* val )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::boolean, std::string( text ), val };
    return *this;
}

menu_builder& menu_builder::add_flag( std::string_view text, int* flags, int val )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::flag, std::string( text ), flags, val };
    return *this;
}

dm_line* menu_builder::build( )
{
    s_line_pool.at( s_total_lines++ ) = { line_type::endmenu };
    return m_start_ptr;
}
